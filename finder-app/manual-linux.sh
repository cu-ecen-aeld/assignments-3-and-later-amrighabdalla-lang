...
cp -p "${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image" "${OUTDIR}/Image"
...
GNUROOT=$(aarch64-none-linux-gnu-gcc -print-sysroot)
cp -a "$GNUROOT"/lib/ld-linux-aarch64.so.1 ${OUTDIR}/rootfs/lib/
cp -a "$GNUROOT"/lib64/libc.so.6 ${OUTDIR}/rootfs/lib64/
cp -a "$GNUROOT"/lib64/libresolv.so.2 ${OUTDIR}/rootfs/lib64/
cp -a "$GNUROOT"/lib64/libm.so.6 ${OUTDIR}/rootfs/lib64/
